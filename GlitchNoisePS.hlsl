...
void main(in PS_IN In, out float4 outDiffuse : SV_Target)
{
    ...
    //ポスタライズ 
    float posterize1 = floor(frac(perlinNoise(_SinTime) * 10.f) / (1.f / Parameter.y)) * (1.f / Parameter.y);
    float posterize2 = floor(frac(perlinNoise(_SinTime) * 5.f) /  (1.f / Parameter.y)) * (1.f / Parameter.y);
