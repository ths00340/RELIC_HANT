...
void main(in PS_EdgeInput In, out float4 outDiffuse : SV_Target)
{
	...
	float3 Normal;
	Normal = g_Texture.Sample(g_SamplerState, In.tex0).xyz * -8.0f;
	Normal += g_Texture.Sample(g_SamplerState, In.tex1.xy).xyz;
	Normal += g_Texture.Sample(g_SamplerState, In.tex2.xy).xyz;
	Normal += g_Texture.Sample(g_SamplerState, In.tex3.xy).xyz;
	Normal += g_Texture.Sample(g_SamplerState, In.tex4.xy).xyz;
	Normal += g_Texture.Sample(g_SamplerState, In.tex5.xy).xyz;
	Normal += g_Texture.Sample(g_SamplerState, In.tex6.xy).xyz;
	Normal += g_Texture.Sample(g_SamplerState, In.tex7.xy).xyz;
	Normal += g_Texture.Sample(g_SamplerState, In.tex8.xy).xyz;
	...
	if (abs(depth-depth2) > 0.001f ) {
		outDiffuse = float4(1.0f, 1.0f, 1.0f, 1.0f);//エッジ
	}
	else {
		outDiffuse = float4(0.f, 0.f, 0.f, 1.f);//エッジ以外
	}
